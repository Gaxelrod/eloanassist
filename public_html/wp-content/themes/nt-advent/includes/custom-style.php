...
function nt_advent_css_options() {
	...
	if ( ot_get_option( 'nt_advent_theme_color_one' ) !='' ) {
		$theCSS .= '::-moz-selection,::selection,.btn-action,.form .subscribe-form .submit-button,.form .btn-action,.signup-form,.signup .btn-action.btn-round,.signup .btn-action,.app .btn-action,.pitch-icon,.feature-sub .sub-inner .btn-action,.btn,.comment-form .submit,#widget-area #searchform input#searchsubmit,body.search article .searchform input[type="submit"],.pager li > a, .pager li > span, .brand .circle,.post-button, .error404 .index .searchform input[type="submit"], .pager li > span, .pager li > a, .section-title:after, .widget-title:after, #widget-area #searchform #searchsubmit {background-color:' . $one_color . ';}';
	...
	if ( $navitem !='' ) 		{ $theCSS .= '.primary-menu  > li > a{color:' . $navitem . ';}'; }
	if ( $navitem_a_s !='' ) 	{ $theCSS .= '.navbar-default.past-main .navbar-nav > li > a{color:' . $navitem_a_s . ';}'; }
	if ( $navitemhover !='' ) 	{ $theCSS .= '.primary-menu  > li > a:hover{color:' . $navitemhover . ' !important;}'; }
	if ( $dropdown_bg !='' ) 	{ $theCSS .= '.primary-menu  > li .dropdown-menu{background-color:' . $dropdown_bg . '!important;}'; }
	if ( $dropdown_item !='' ) 	{ $theCSS .= '.primary-menu  > li .dropdown-menu > li > a{color:' . $dropdown_item . '!important;}'; }
	if ( $dropdown_i_h !='' ) 	{ $theCSS .= '.primary-menu  > li .dropdown-menu> li > a:hover{color:' . $dropdown_i_h . ' !important;}'; }
	...
	// sidebar
    $sb_bg 					= esc_attr( ot_get_option( 'nt_advent_sb_bg' ) );
    $sb_t_c 				= esc_attr( ot_get_option( 'nt_advent_sb_t_c' ) );
    $sb_c 					= esc_attr( ot_get_option( 'nt_advent_sb_c' ) );
    $sb_l_c 				= esc_attr( ot_get_option( 'nt_advent_sb_l_c' ) );
    $sb_l_c_h 				= esc_attr( ot_get_option( 'nt_advent_sb_l_c_h' ) );
    $sb_s_t 				= esc_attr( ot_get_option( 'nt_advent_sb_s_t' ) );
    $sb_s_bg 				= esc_attr( ot_get_option( 'nt_advent_sb_s_bg' ) );
	...
	if ( $nt_advent_page_bg !='' ) 			{  $theCSS .= '.page-id-' . get_the_ID().' .index-header {background-image: url(' . esc_url( $nt_advent_page_bg ) .') !important; }';  }
	if ( $nt_advent_page_bg_color !='' && $nt_advent_page_bg =='' ) 	{  $theCSS .= '.page-id-' . get_the_ID().' .index-header {background-image: none !important; }';  }
	if ( $nt_advent_page_bg_color !='' ) 	{  $theCSS .= '.page-id-' . get_the_ID().' .index-header {background-color: ' . $nt_advent_page_bg_color .' !important; }';  }
	if ( $nt_advent_page_text_color !='' ) 	{  $theCSS .= '.page-id-' . get_the_ID().' .index-header h2, .page-' . get_the_ID().' .index-header p {color: ' . $nt_advent_page_text_color .' !important; }';  }
	if ( $nt_advent_header_p_top !='' ) 	{  $theCSS .= '.page-id-' . get_the_ID().' .index-header  {padding-bottom : ' . $nt_advent_header_p_top .'px !important; }';  }
	if ( $nt_advent_header_p_bottom !='' ) 	{  $theCSS .= '.page-id-' . get_the_ID().' .index-header  {padding-top : ' . $nt_advent_header_p_bottom .'px !important; }';  }
    ...
    $theCSS .= '.single .index-header  h1{color: '.  esc_attr( ot_get_option( 'nt_advent_singleheadingcolor' ) ) .'; }'; 
    }
	if  ( ot_get_option( 'nt_advent_single_heading_fontsize' ) !='' ){
    $theCSS .= '.single .index-header  h1{font-size: '.  esc_attr( ot_get_option( 'nt_advent_single_heading_fontsize' ) ) .'px; }'; 
    }
	...
	if  ( ot_get_option( 'nt_advent_archiveheadingcolor' ) !='' ){
     $theCSS .= '.archive .index-header  h1{color: '.  esc_attr( ot_get_option( 'nt_advent_archiveheadingcolor' ) ) .'; }'; 
	}
	if  ( ot_get_option( 'nt_advent_archive_heading_fontsize' ) !='' ){
     $theCSS .= '.archive .index-header  h1{font-size: '.  esc_attr( ot_get_option( 'nt_advent_archive_heading_fontsize' ) ) .'px; }'; 
    }
	...
	if  ( ot_get_option( 'nt_advent_errorheadingcolor' ) !='' ){
     $theCSS .= '.error404 .index-header  h1{color: '.  esc_attr( ot_get_option( 'nt_advent_errorheadingcolor' ) ) .'; }'; 
	}
	if  ( ot_get_option( 'nt_advent_error_heading_fontsize' ) !='' ){
     $theCSS .= '.error404 .index-header  h1{font-size: '.  esc_attr( ot_get_option( 'nt_advent_error_heading_fontsize' ) ) .'px; }'; 
    }
	...
	if  ( ot_get_option( 'nt_advent_searchheadingcolor' ) !='' ){
     $theCSS .= '.search .index-header  h1{color: '.  esc_attr( ot_get_option( 'nt_advent_searchheadingcolor' ) ) .'; }'; 
	}
	if  ( ot_get_option( 'nt_advent_search_heading_fontsize' ) !='' ){
     $theCSS .= '.search .index-header  h1{font-size: '.  esc_attr( ot_get_option( 'nt_advent_search_heading_fontsize' ) ) .'px; }'; 
    }
	...
	if  ( ot_get_option( 'nt_advent_blogbreadcrubmscolor' ) !='' ){
     $theCSS .= 'a.breadcrumb-item{color: '.  esc_attr( ot_get_option( 'nt_advent_blogbreadcrubmscolor' ) ) .'; }'; 
	}
	if  ( ot_get_option( 'nt_advent_blogbreadcrubmshovercolor' ) !='' ){
     $theCSS .= 'a.breadcrumb-item:hover{color: '.  esc_attr( ot_get_option( 'nt_advent_blogbreadcrubmshovercolor' ) ) .'; }'; 
	}
	if  ( ot_get_option( 'nt_advent_blogbreadcrubmscurrentcolor' ) !='' ){
     $theCSS .= '.breadcrumb-current{color: '.  esc_attr( ot_get_option( 'nt_advent_blogbreadcrubmscurrentcolor' ) ) .'; }'; 
	}
